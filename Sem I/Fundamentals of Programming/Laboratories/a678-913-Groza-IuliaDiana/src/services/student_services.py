...
class StudentServices:
    ...
    def best_students(self):
        ...
        for student in self._students.list:
            students.update({(student.id, student.name): self.compute_gpa(self.compute_all_averages(student.id))})

        sorted_students = {}
        sorted_keys = sorted(students, key=students.get, reverse=True)

        for key in sorted_keys:
            sorted_students[key] = students[key]
