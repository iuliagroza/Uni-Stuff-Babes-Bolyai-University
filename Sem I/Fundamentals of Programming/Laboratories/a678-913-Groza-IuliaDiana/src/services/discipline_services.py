...
class DisciplineServices:
    ...
    def generate_disciplines_list(self):
        """
    ...
    def best_disciplines(self):
        ...
        for discipline in self._disciplines.list:
            disciplines.update({(discipline.id, discipline.name): self.compute_all_averages(discipline.id)})

        sorted_disciplines = {}
        sorted_keys = sorted(disciplines, key=disciplines.get, reverse=True)

        for key in sorted_keys:
            sorted_disciplines[key] = disciplines[key]
