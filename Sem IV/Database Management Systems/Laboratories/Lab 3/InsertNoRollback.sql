...
CREATE OR ALTER PROCEDURE addRowGamesRecover @name VARCHAR(50), @description VARCHAR(300), @yearOfRelease INT, @noOfPlayers INT, @rating INT, @noOfDownloads INT, @vidName VARCHAR(50), @genName VARCHAR(50) AS
BEGIN
	SET NOCOUNT ON
	BEGIN TRAN
	BEGIN TRY
	...
	BEGIN CATCH
		ROLLBACK TRAN
		EXEC sp_log_changes '', '', 'Rolledback Games data.', ''
	END CATCH
END

GO
CREATE OR ALTER PROCEDURE addRowUsersRecover  @name VARCHAR(50), @email VARCHAR(50), @age INT, @country VARCHAR(50), @subscribedToNewsletter BIT AS
BEGIN
	SET NOCOUNT ON
	BEGIN TRAN
	BEGIN TRY
	...
	BEGIN CATCH
		ROLLBACK TRAN
		EXEC sp_log_changes '', '', 'Rolledback Users data.', ''
	END CATCH
END

GO
CREATE OR ALTER PROCEDURE addUsersGamesRecover @userName VARCHAR(50), @gameName VARCHAR(50) AS
BEGIN
	SET NOCOUNT ON
	BEGIN TRAN
	BEGIN TRY
	...
	BEGIN CATCH
		ROLLBACK TRAN
		EXEC sp_log_changes '', '', 'Rolledback UsersGames data.', ''
	END CATCH
END
