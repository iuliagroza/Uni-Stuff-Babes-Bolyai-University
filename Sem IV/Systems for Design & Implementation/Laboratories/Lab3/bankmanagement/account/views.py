...
class AccountsByNumberOfCustomers(generics.ListCreateAPIView):
    serializer_class = AccountSerializer

    def get_queryset(self):
        query = Account.objects.annotate(
            no_customers=Count(
                Customer.accounts.through.objects.filter(account=OuterRef("pk"))
                .exclude(customer=OuterRef("customers__id"))
                .values("customer")
                .distinct(),
                distinct=True,
            )
        ).order_by("-no_customers")
