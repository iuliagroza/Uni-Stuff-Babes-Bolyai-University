...
void UI::tinder() {
    ...
    while (index < this->controller.get_admin_repo().get_movies().size()) {
        int pos = this->controller.get_user_repo()->find(this->controller.get_admin_repo().get_movies()[index]);
        if (pos == -1 &&
...
void UI::remove_user() {
    ...
    try {
        ...
        std::string response = get_response_input();
        if (response == "yes") {
            int pos = this->controller.get_admin_repo().find(Movie("a", "a", 0, 0, trailer));
