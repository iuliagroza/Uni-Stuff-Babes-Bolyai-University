...
void testFilter() {
    ...
    for(int val=0; val<10; val++) {
        list.add(val);
        assert(list.getElement(val) == val);
    }

    list.filter(filter_cond);

    assert(list.search(0) == -1);
    assert(list.search(2) == -1);
    assert(list.search(4) == -1);
    assert(list.search(6) == -1);
    assert(list.search(8) == -1);
    assert(list.search(1) == 0);
    assert(list.search(3) == 1);


    SortedIndexedList no_need_to_filter_list = SortedIndexedList(desc);
    for(int val=1; val<10; val+=2) {
        no_need_to_filter_list.add(val);
    }

    no_need_to_filter_list.filter(filter_cond);
    assert(no_need_to_filter_list.search(1) == 4);
    assert(no_need_to_filter_list.search(3) == 3);
    assert(no_need_to_filter_list.search(2) == -1);
}
