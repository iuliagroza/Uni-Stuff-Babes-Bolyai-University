...
def read_graph(file_path):
    """
    Function that reads a graph from a text file with the path file_path. FileNotFoundError occurs if
...
def read_activities(file_path):
    ...
    and the duration of the activity. The associated graph is constructed and returned. FileNotFoundError occurs if
    file_path does not exist.
    :param file_path:
    :return: DirectedGraph object
    """
        ...
        with open(file_path, "r") as file:
            ...
            for line in lines[1:]:
                ...
                for vertex in prerequisites:
                    G.add_edge_valid(int(vertex), activity, 0)
