...
from external_functions import generate_random_graph, print_graph, lowest_cost_walk_dp, negative_cycle_detection, \
    read_activities
...
1. Read from a file a list of activities with duration and list of prerequisites for each of them (and overwrite the current graph).
...
19. Check if the graph is a DAG and perform a topological sorting.
20. Print the earliest and the latest starting time for each activity and the total time of the project.
21. Print the critical activities.
22. Exit.""")
    ...
    def get_option():
        while True:
            try:
                ipt = input(">>> ")
                if not re.match("(^[1-9]$)|(^1[0-9]$)|(^2[0-2]$)", ipt):
                    raise InputError
                break
            except InputError:
                print("Invalid input.")
    ...
    def read(self):
        print("Please type the name of the file (input.txt, input2.txt):")
        while True:
            try:
                file_path = input(">>> ")

                if not re.match("(^input.txt$)|(^input2.txt$)",
                                file_path):
                    raise InputError
                break
            except InputError:
                print("Invalid input.")
        self.__g = read_activities(file_path)
    ...
    def lowest_cost_walk(self):
        ...
        if negative_cycle_detection(self.__g, source):
            ...
            print(
                f"The length of the lowest cost walk from {source} to {target} is of length {len(walk) - 1} and of cost {minimum_cost}")
            print("The walk is:")
            for i in range(len(walk) - 1):
                print(walk[i], end="-")
            print(walk[-1])

    def dag_and_topological_sorting(self):
        sorted_vertices = self.__g.topological_sorting()

        if len(sorted_vertices) == 0:
            print("The graph is not a DAG.")
        else:
            print("The topological sorting of the graph is:")
            print(sorted_vertices)

    def earliest_and_latest_times(self):
        earliest_start, earliest_end, latest_start, latest_end, duration = self.__g.get_times()
        ...
        if duration == -1:
            ...
            for i in range(self.__g.get_number_of_vertices()):
                table_list.append([str(i), str(earliest_start[i]) + "-" + str(earliest_end[i]),
                                   str(latest_start[i]) + "-" + str(latest_end[i])])
    ...
    def critical_activities(self):
        critical = self.__g.critical_activities()
        if not critical:
            print("The graph is not a DAG.")
        else:
    ...
    def start(self):
        while True:
            ...
            if option == 1:
                ...
                self.dag_and_topological_sorting()
            elif option == 20:
                self.earliest_and_latest_times()
            elif option == 21:
